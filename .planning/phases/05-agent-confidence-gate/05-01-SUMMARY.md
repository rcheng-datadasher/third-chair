...
key-decisions:
  ...
  - "Follow-up (quick task, same day): replaced the 0-1 freehand CONFIDENCE_RUBRIC with a user-defined 0-10 rubric that is the SUM of four reported dimensions (intent_points 0-3, time_points 0-3, participant_points 0-2, fitness_points 0-2). `confidence` (0-1) is now strictly derived (`score / 10`) by `reconcileScore` in lib/agent/extract-intents.ts, never trusted from the model even when the model returns one; a model score/points mismatch is recomputed from the points and logged. `bucketConfidence`/`ConfidenceBand` in lib/agent/graph.ts moved from HIGH_FROM=0.75/LOW_BELOW=0.4 on a 0-1 confidence to a new `bandForScore(score)` on the 0-10 score: HIGH_FROM=9 -> \"high\" (approval card), SHOW_FROM=7 -> \"medium\" (Edit & approve card), score<=6 -> \"ignored\" (silent, is_actionable forced false, never posts to Slack) — the show-only-if->6 gate. Verified with an offline logic check (hand-applied rubric scores for the same 6 p5-samples) since this worktree had no reachable AI_API_KEY/AI_BASE_URL to re-run the live MODEL_FAST call; all 6 landed on their expected band and the score/confidence/is_actionable derivation was confirmed correct, including the mismatch-recovery path."
